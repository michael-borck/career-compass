...
import { useSessionStore, type IndustryExploration } from '@/lib/session-store';
...
type Props = { initialIndustry?: string };

export default function IndustryInputCard({ initialIndustry }: Props) {
  ...
  async function handleRun() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || 'Industry exploration failed');
      }
      const { exploration, trimmed: wasTrimmed } = (await res.json()) as {
        exploration: IndustryExploration;
        trimmed?: boolean;
      };
      store.setIndustryExploration(exploration);
      ...
      <div className='border border-border rounded-lg bg-paper p-6'>
        ...
        <div className='space-y-4'>
          ...
          <div className='flex justify-center pt-2'>
            <Button onClick={handleRun} disabled={!industry.trim() || running}>
              {running ? (
