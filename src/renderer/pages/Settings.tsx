...
export default function Settings() {
  ...
  const [connectionStatus, setConnectionStatus] = useState<{
    [key: string]: { success: boolean; error?: string | null };
  }>({});
  ...
  const loadModels = async () => {
    ...
    try {
      // Renderer is always Electron now; previously this had a web-mode fetch
  ...
  const testConnection = async (provider: LLMProvider) => {
    try {
      ...
      if (result.success) {
        ...
        toast.error(`Could not connect: ${result.error ?? 'Unknown error'}`);
      }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : 'Unknown error';
      setConnectionStatus((prev) => ({
        ...prev,
        [provider]: { success: false, error: errorMsg },
      }));
  ...
  const checkEnvVar = async (provider: LLMProvider): Promise<boolean> => {
    ...
    if (typeof window !== 'undefined' && window.electronAPI) {
      const val = await window.electronAPI.getEnvVar(envVar);
      return !!val;
    }
    return false;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (!settings.model.trim()) {
        toast.error(
          'Please select a model first. Click "Refresh models" to see what is available.'
        );
        setIsLoading(false);
        return;
      }
      ...
      if (
        ...
        if (!hasEnvVar) {
          ...
          toast.error(
            `No secret key found. Paste one above or set the ${envVar} environment variable.`
          );
          setIsLoading(false);
          return;
        }
      ...
      // apiKey lives in secureStorage, not the settings blob.
      delete (settingsToSave as Partial<SettingsConfig>).apiKey;
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {connectionStatus[settings.provider] && (
                  <div
                    className={`mt-2 p-3 rounded-lg text-[var(--text-sm)] border ${
