...
export function buildCoverLetterPrompt(input: CoverLetterInput): string {
  ...
  sections.push(
    'Write a professional cover letter (300-500 words) for a student applying to a role. The letter should be formal but human. Written in first person.'
  );
  sections.push(
...
export function parseCoverLetter(raw: string): CoverLetterOutput {
  ...
  if (typeof parsed.greeting !== 'string' || !parsed.greeting.trim())
    throw new Error('parseCoverLetter: missing greeting');
  if (typeof parsed.body !== 'string' || !parsed.body.trim())
    throw new Error('parseCoverLetter: missing body');
  if (typeof parsed.closing !== 'string' || !parsed.closing.trim())
    throw new Error('parseCoverLetter: missing closing');
  return {
