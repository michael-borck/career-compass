...
export function parsePitch(raw: string): PitchOutput {
  ...
  if (typeof parsed.hook !== 'string' || !parsed.hook.trim())
    throw new Error('parsePitch: missing hook');
  if (typeof parsed.body !== 'string' || !parsed.body.trim())
    throw new Error('parsePitch: missing body');
  if (typeof parsed.close !== 'string' || !parsed.close.trim())
    throw new Error('parsePitch: missing close');
  if (typeof parsed.fullScript !== 'string' || !parsed.fullScript.trim())
    throw new Error('parsePitch: missing fullScript');
  return {
